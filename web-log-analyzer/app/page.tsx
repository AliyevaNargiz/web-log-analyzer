...
export default function HomePage() {
  ...
  const uploadToBackend = async (file: File): Promise<string | null> => {
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = errorData?.error || errorData?.message || errorData?.details?.[0] || 'Upload failed. Please check the file format.'
  ...
  const runClientAnalysis = async (selectedAnalyses: string[]): Promise<AnalysisResult> => {
    ...
    if (selectedAnalyses.includes("http-methods")) {
      results.analyses.httpMethods = Object.entries(
        filteredData.reduce(
          (acc, e) => {
    ...
    if (selectedAnalyses.includes("daily-traffic")) {
      ...
      results.analyses.dailyTraffic = dayNames.map((day, index) => ({
        day,
        dayOfWeek: index + 1, // Spark uses 1-7
        count: dailyData[index] || 0,
      }))
    }

    if (selectedAnalyses.includes("error-rate")) {
      const totalRequests = filteredData.length
      const clientErrors = filteredData.filter((e) => e.status >= 400 && e.status < 500).length
      const serverErrors = filteredData.filter((e) => e.status >= 500).length
      const success = filteredData.filter((e) => e.status >= 200 && e.status < 300).length
      const redirects = filteredData.filter((e) => e.status >= 300 && e.status < 400).length
      ...
      results.analyses.errorRate = {
        ...
        success,
        redirects,
        clientErrorRate: totalRequests > 0 ? (clientErrors / totalRequests) * 100 : 0,
        serverErrorRate: totalRequests > 0 ? (serverErrors / totalRequests) * 100 : 0,
        successRate: totalRequests > 0 ? (success / totalRequests) * 100 : 0,
        topErrors,
      }
    }

    if (selectedAnalyses.includes("response-size")) {
      const sizes = filteredData.map((e) => e.size).filter((s) => s > 0)
      if (sizes.length > 0) {
        sizes.sort((a, b) => a - b)
        const min = sizes[0]
        ...
        const p99Index = Math.floor(sizes.length * 0.99)
        const p95 = sizes[p95Index] || 0
        const p99 = sizes[p99Index] || 0
    ...
    if (selectedAnalyses.includes("protocol-version")) {
      results.analyses.protocolVersion = Object.entries(
        filteredData.reduce(
          (acc, e) => {
    ...
    if (selectedAnalyses.includes("peak-traffic")) {
      const hourlyData: Record<number, number> = {}
      const dailyData: Record<number, number> = {}
      const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
      ...
      const peakDays = Object.entries(dailyData)
        ...
        .slice(0, 3)
        .map(([day, count]) => ({
          day: dayNames[Number.parseInt(day)],
          dayOfWeek: Number.parseInt(day) + 1,
          count,
        }))
