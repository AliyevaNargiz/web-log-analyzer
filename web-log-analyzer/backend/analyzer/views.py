...
def upload_file(request):
    ...
    # Validate file type - support .txt, .log, and .csv files
    if not any(filename.endswith(ext) for ext in ['.txt', '.log', '.csv']):
        logger.warning(f"Invalid file type: {filename}")
        return Response(
            {'error': 'Only TXT, LOG, and CSV files are supported'},
            status=status.HTTP_400_BAD_REQUEST
        )
...
def download_result(request, result_id):
    ...
    try:
        ...
        file_map = {
            ...
            'http-methods': 'http_methods.csv',
            'httpMethods': 'http_methods.csv',
            'daily-traffic': 'daily_traffic.csv',
            'dailyTraffic': 'daily_traffic.csv',
            'error-rate': 'error_rate.csv',
            'errorRate': 'error_rate.csv',
            'top-errors': 'top_errors.csv',
            'topErrors': 'top_errors.csv',
            'response-size': 'response_size.csv',
            'responseSize': 'response_size.csv',
            'size-distribution': 'size_distribution.csv',
            'sizeDistribution': 'size_distribution.csv',
            'protocol-version': 'protocol_version.csv',
            'protocolVersion': 'protocol_version.csv',
            'peak-traffic': 'peak_traffic.csv',
            'peakTraffic': 'peak_traffic.csv',
            'json': 'results.json',
        }
